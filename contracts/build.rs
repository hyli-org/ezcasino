...
#[cfg(all(
    feature = "build",
    not(any(feature = "blackjack", feature = "session-key-manager"))
))]
fn main() {
    compile_error!("When the 'build' feature is enabled, at least one of the following features must also be enabled: all, blackjack, session-key-manager.");
}

#[cfg(all(
    not(clippy),
    feature = "build",
    any(feature = "blackjack", feature = "session-key-manager",)
))]
fn main() {
    trait CodegenConsts {
    ...
    let methods: Vec<GuestListEntry> = [
        #[cfg(feature = "blackjack")]
        "blackjack",
        #[cfg(feature = "session-key-manager")]
        "session-key-manager",
    ]
    .iter()
    .map(|name| {
