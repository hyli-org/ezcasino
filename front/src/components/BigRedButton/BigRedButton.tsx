...
const BigRedButton: React.FC<BigRedButtonProps> = ({ onClose }) => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (
      ...
      e.target.closest(".win95-title-bar")
    ) {
      setIsDragging(true);
      setDragStart({
  ...
  const handleButtonPress = async () => {
    ...
    const windowElement = buttonRef.current?.closest(
      ".win95-window"
    ) as HTMLElement | null;
    ...
    const distance = Math.sqrt(
      Math.pow(randomX - position.x, 2) + Math.pow(randomY - position.y, 2)
    );
    ...
    // Send blob tx
    if (wallet?.address) {
      const blobTransfer = blob_builder.smt_token.transfer(
        "faucet",
        wallet.address,
        "oranj",
        BigInt(1),
    ...
    <div
      ...
      style={{
        ...
        transition: isDragging
          ? "none"
          : `left ${transitionDuration}s cubic-bezier(0.68, -0.55, 0.265, 1.55), top ${transitionDuration}s cubic-bezier(0.68, -0.55, 0.265, 1.55)`,
      }}
                ...
                style={
                  {
                    ...
                    "--random-rotation": `${
                      orange.rotation + (Math.random() > 0.5 ? 180 : -180)
                    }deg`,
